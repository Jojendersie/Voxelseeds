            ...
            if (gen < 1000)
            {
                ...
                if (gen < TypeInformation.GetGrowHeight(VoxelType.BEECH_WOOD))
                {
                    if (neighbourhood[1, 1, 1].From == Direction.DOWN)
                    {
                        ...
                        // Grow to the sides
                        if (CanPlace(0, 1, 0, neighbourhood)) output[0, 1, 0] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.BACK_RIGHT);
                        if (CanPlace(1, 1, 0, neighbourhood)) output[1, 1, 0] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.RIGHT);
                        if (CanPlace(2, 1, 0, neighbourhood)) output[2, 1, 0] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.FOR_RIGHT);
                        if (CanPlace(0, 1, 1, neighbourhood)) output[0, 1, 1] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.BACK);
                        if (CanPlace(2, 1, 1, neighbourhood)) output[2, 1, 1] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.FOR);
                        if (CanPlace(0, 1, 2, neighbourhood)) output[0, 1, 2] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.BACK_LEFT);
                        if (CanPlace(1, 1, 2, neighbourhood)) output[1, 1, 2] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.LEFT);
                        if (CanPlace(2, 1, 2, neighbourhood)) output[2, 1, 2] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen, 0, 0, Direction.FOR_LEFT);
                    ...
                    if (CanPlace(1, 0, 1, neighbourhood))
                        output[1, 0, 1] = new VoxelInfo(VoxelType.BEECH_WOOD, true, gen + Random.Next(4), 0, 0, Direction.UP);
                }
                else
                {
